...
sudo DEBIAN_FRONTEND=noninteractive apt-get -yq install qjackctl qjackrcd qpwgraph jackd2
...
sudo apt install -y invada-studio-plugins-ladspa ir.lv2 amb-plugins autotalent blepvco blop blop-lv2 bs2b-ladspa cmt csladspa fil-plugins mcp-plugins omins rev-plugins ste-plugins swh-plugins tap-plugins vco-plugins vlevel a2jmidid gmidimonitor jack-keyboard jackd japa jconvolver jkmeter jmeters jnoise klick meterbridge qjackctl qjackrcd qmidiarp qtractor radium-compressor rotter fst-dev freewheeling horgand muse nama gxtuner sox terminatorx
